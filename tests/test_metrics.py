...
def _let_at(day, band, units=1, revenue=3_000, keyed_at="08:30:00"):
    """A let that happened in `band`, keyed in the next morning at `keyed_at`.

    The two are deliberately unrelated: that gap is the whole point.
    """
    return {"timestamp": f"2026-06-{day:02d} {keyed_at}", "room_type": "short time",
            "quantity": 1, "nights": units, "total_revenue": units * revenue,
            "daypart": band}


def test_the_band_is_read_from_the_booking_never_from_the_clock():
    """Bookings are keyed in the morning after; the timestamp is the typing."""
    assert metrics.daypart_of({"daypart": "Evening"}) == "Evening"
    assert metrics.daypart_of({"daypart": "evening"}) == "Evening"
    assert metrics.daypart_of({"daypart": "whenever"}) is None
    assert metrics.daypart_of({}) is None


def test_a_morning_of_data_entry_is_not_a_morning_trade():
    """The bug this replaced: 81 evening lets keyed at 08:30 read as Morning."""
    rows = [{"timestamp": f"2026-06-{d:02d} 08:30:00", "room_type": "short time",
             "quantity": 1, "nights": 3, "total_revenue": 9_000}
            for d in range(2, 29)]
    s = metrics.daypart_split(rows, *_JUNE, hours_map=_HOURLY)
    assert s.untimed_lets == 81
    assert all(b.lets == 0 for b in s.bands)     # no band invented from the clock
    assert metrics.daypart_verdict(s) == ("", "")


def test_a_booking_with_no_band_recorded_is_untimed():
...
def test_lets_are_grouped_by_when_in_the_day_they_happened():
    rows = [_let_at(d, band, n) for d in range(1, 29)
            for band, n in (("Morning", 1), ("Afternoon", 2), ("Evening", 4))]
    s = metrics.daypart_split(rows, *_JUNE, hours_map=_HOURLY)
...
def test_overnight_stays_are_left_out_of_the_daypart_split():
    ...
    rows = [_let_at(1, "Evening"), {"timestamp": "2026-06-01 20:00:00",
...
def test_a_lopsided_day_says_which_band_to_reprice():
    rows = ([_let_at(d, "Evening", 4) for d in range(1, 29)]
            + [_let_at(d, "Morning", 1) for d in range(1, 29)])
...
def test_an_even_day_says_one_price_is_the_right_shape():
    rows = [_let_at(d, b, 2) for d in range(1, 29)
...
def test_a_band_already_priced_higher_is_told_it_is_holding():
    rows = ([_let_at(d, "Evening", 4, revenue=5_000) for d in range(1, 29)]
            + [_let_at(d, "Morning", 1, revenue=3_000) for d in range(1, 29)])
    verdict, _ = metrics.daypart_verdict(
        metrics.daypart_split(rows, *_JUNE, hours_map=_HOURLY))
...
def test_too_few_timed_lets_says_nothing_at_all():
    s = metrics.daypart_split([_let_at(1, "Evening")], *_JUNE, hours_map=_HOURLY)
